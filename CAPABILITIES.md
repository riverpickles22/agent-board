...
this works" before anyone starts building. Once a story is built it
carries **How to test this yourself** — numbered steps in plain words
(open this, do that, you should see this) written for the person who did
not build it — shown on the story card, in its hover peek and in the
epic's markdown copy; `doctor` flags a story sitting in review without
them.
...
process serves them all on one port — `/<name>/#/board`,
`/<other-name>/#/ideas` — and
...
- **Multi-project** — `projects.json` registers name → data dir; one
  codebase serves every project you register, agent-board's own dogfood
  backlog included.
