...
| Meta row | `metaRowEl()` `.imeta-row` | labeled micro-pills: priority, category, milestone (`short()`, full in title), theme, tags; `⊸ depends on` links; dim created/updated dates right | every dep that resolves renders as a `.dep-l` chip button: a **done** dep is vivid (`.met` — `--st-ready` fill, ✓ prefix) because a cleared dependency is the good news; an unfinished one is muted (`--ink-3` on a plain border) so the open ones don't shout. Click an idea dep (`data-goidea`) → that idea's deep dive; click an epic dep (`data-goepic`) → `goToEpic()` jumps to it on the board and opens its card. Ids that match nothing render inert (`.dep-x`, dotted). Past `DEP_ROW_MAX` (4) chips the row collapses to one `.dep-sum` chip reading **X/Y completed** (vivid only when all landed); click expands it to the full chip set (`depsExpanded`, per idea, for the session) — the count is the fact you wanted, the chips are the way in. The card's hover peek carries the full list (`pkDeps`, system.md) |
