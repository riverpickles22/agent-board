...
| Card peek | `wirePeek()` `showPeek()` `.peek` `PEEK_DELAY` | dwell **1s** on any epic, story or idea card and a popover appears beside it with what the card had no room for: full description, acceptance criteria or story list, claim, and a footer of the small facts | the board is a scanning surface, and opening a card to answer "what is this again?" costs a modal and a click back. Not a `title` tooltip: those are unstyled, truncate, and cannot render the computed state chip. Not instant either — a popover that fires while you sweep the board is noise, so it waits for a dwell that means *this one*. `pointer-events:none`, so it can never intercept a click or a drag; the left border takes the card's own colour (lane for board cards, stage for ideas). Anything that means "I have moved on" — mousedown, dragstart, wheel, scroll, keydown, leaving the card — cancels it, dwell timer included |
