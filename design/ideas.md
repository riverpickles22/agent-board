...
│ (Active 11)(Done & rejected 3) │LENS (Status)(Theme)(Leverage)         │
│                                  (⧉ copy) (↻ refresh)  [+ New idea]  │
│                                                                      │
│ IDEA                READY FOR REVIEW 1       READY TO IMPLEMENT 0    │
│ FRONT BURNER 4      ┏━━━━━━━━━━━━━━━━━━━┓    none                    │
...
| Controls row | `.ideas-toggle` | its own row under the description: "Active (N)" / "Done & rejected (N)" chips on the left, "+ New idea" pushed right (`margin-left:auto`) | toggle flips `ideasArchive` (in-memory, defaults to active) and re-renders; each N counts every idea in that view across all its columns — the Active chip is the board's total, not just the Idea column |
...
| Status column | `.ideas-col` | label + count per view's status list — except the Idea column, which shows no count (its burner headers count their sections; the Active chip counts the pipeline) | active: `IDEA_STATUS` (idea → ready for review → ready to implement); archive: `IDEA_STATUS_ARCHIVE` (done, rejected). "Ready for review" is the agent's hand-off; "ready to implement" is the human's thumbs-up (AGENTS.md §2). Legacy statuses (considering/planned/building) render via `ideaStatus()` normalization |
...
| Dependencies line | `.idep` `depRowEl(i)` `.dep-l` | `⊸ depends on:` then one chip **button** per id — a done dep vivid (`.met`, `--st-ready` fill, ✓), an open one muted; past `DEP_ROW_MAX` (4) a single **X/Y completed** chip that expands on click | renders **only when `deps` is non-empty**; same chips and rules as the deep dive's meta row (idea-detail.md) so card and page agree. Click an idea dep → its deep dive, an epic dep → the board with its card open — the click is stopped (`wireDepChips`) so it never opens *this* card, and mousedown is stopped so it never starts a drag. Unknown ids render dim-dotted rather than vanishing. Open deps stay **muted, never loud** — idea deps are informational sequencing with no ordering rule, so they must not look like the board's dependency violations |
