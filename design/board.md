...
| Rows: row header | `.swim-h` | epic id, name, gate badge (`.egate`, when gate closed), done-count "d/N {last lane}", and a `⧉` copy button (`.rowcopy`, also on the folded row) | click → `openEdit(id)`; the buttons own their clicks |
...
| Epic copy | `epicMarkdown()` `copyEpic()` `.rowcopy` `#m-copy` | one click — row header or epic modal — puts the whole epic on the clipboard as markdown: description, notes, test plan, meta line, then **every story** with kind/lane/ready, description, acceptance criteria and context; ids in backticks so answers can reference cards back | the Ideas page's copy-as-markdown convention at the epic level: a groomed unit of work pastes into another tool without opening each story. Toast confirms, matching the Ideas convention |
