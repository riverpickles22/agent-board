...
```sh
./board install    # put `board` on your PATH (once)
board              # open your default board — asks which one, then remembers
board <name>       # serve a registered project's backlog
board list         # list registered projects
board stop         # stop the board on :4300 (board stop 4301 for another)
```

`./board install` symlinks this launcher into `~/.local/bin`, so `board`
works from any directory — every command below reads the same either way.
The first bare `board` asks which project it should open and remembers the
answer in `~/.config/agent-board/config.json`; `board default <name>` changes
it, `board default --none` clears it, and `BOARD_DEFAULT=<name> board`
overrides it for one run. Naming a project (`board <name>`) never changes the
default.
...
port. Run a second board at the same time with `PORT=4301 ./board <name>`.
Needs only Node.js (v18+) — no install step, no dependencies, and it
binds to `127.0.0.1` only.

**Many boards, one server.** `./board <name>` serves every project in
[`projects.json`](projects.json) from a single port and opens on the one
you named — `/<name>/#/board`, `/<other-name>/#/ideas` — and a switcher in
the header moves between them with no restart and no second port. The project lives in
...
`./board` reads `projects.json`, a simple name → data-directory map. The
paths in it are absolute and machine-specific, so it is **gitignored** — your
registry is yours. A fresh clone has none; `./board new <name> <dir>` writes
one for you, or copy [`projects.example.json`](projects.example.json) and
edit it.
...
|---|---|
| `<name>` | `<name>/<name>-system-design/roadmap/` — one row per registered project |
| board | `agent-board/roadmap/` — this repo, dogfooding its own backlog (gitignored, like any other project's data) |
...
`./board new <name> <dir>` does all four steps below — it scaffolds the data
directory with sensible defaults, adopts any files already there, and
registers the name in `projects.json` (creating that file on a fresh clone).
By hand:
...
3. Register it in `projects.json` with a short name — copy
   [`projects.example.json`](projects.example.json) if you don't have one yet.
4. `./board <name>`.
